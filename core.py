...
def BnNLInception(x, nb_inc=16, inc_filt_list=[(1,1), (3,3), (5,5)], name='uinc') :

    uc_list = []
    for idx, ftuple in enumerate( inc_filt_list ) :
        uc = QuadraticLayer(nb_inc, ftuple, activation='linear', padding="SAME", strides=(1, 1))(x)
        uc_list.append(uc)
    if (len( uc_list ) > 1) :
        uc_merge = Concatenate( axis=-1, name=name+'_merge')(uc_list)
    else :
...
def create_linear_BusterNet( weight_file=None ) :
...
def create_RobusterNet(busternet, weight_file=None ) :
    merged_feat = Concatenate(axis=-1, name='mergenl')(
        [busternet.layers[2].get_output_at(-1), busternet.layers[3].get_output_at(-1)])
    bn = BatchNormalization()(merged_feat)
    f = BnNLInception(bn, 3, name='nlInception')
    mask_out = Conv2D(3, (3, 3), padding='same', activation='softmax', name='pred_masknl')(f)
    mask_out = ResizeBack(name='restorenl')([mask_out, busternet.input])
    model = Model(inputs=busternet.input, outputs=mask_out, name='nlBusterNet')
